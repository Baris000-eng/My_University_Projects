    ...
    (lambda (exp env)
      (cases expression exp
        ...
        (proc-nested-exp
         ...
         (proc-val (nested-procedure var name body count  env))
       )        
        ...
        (letrec-nested-exp
         (p-name b-var b-count p-body letrec-body)
   (value-of letrec-body
           (extend-env-rec-nested p-name b-var p-body 'count env))
    ...
    (lambda (proc1 arg)
      (cases proc proc1
        ...
        ; #####################################################
         ...
         (nested-procedure (bvar name body count env)
                        
                             (recursive-displayer name (expval->num (apply-env  (extend-env 'count (num-val (+ (expval->num (apply-env env count)) 1)) env) count))) 
                          
                    (value-of body (extend-env bvar arg (extend-env-rec-nested name bvar body count (extend-env 'count (num-val (+ (expval->num (apply-env env count)) 1)) env) )))                
